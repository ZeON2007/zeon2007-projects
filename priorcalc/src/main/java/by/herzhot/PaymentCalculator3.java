...
public class PaymentCalculator3 {
    ...
    public static void main(String[] args) {
        ...
        final int creditDuration = 12 * years - 1;
        ...
        for (int i = 0; i < firstMonthDuration; i++) {
            payments[i] = 0;
            percents[i] = ((contractAmount * (refundingRate - rateOffset) / 1200) * (daysInMonth - contractDay) / daysInMonth);
            amounts[i] = payments[i] + percents[i];
        }

        for (int i = firstMonthDuration; i < onlyPercentDuration; i++) {
            payments[i] = 0;
            percents[i] = contractAmount * (refundingRate - rateOffset) / 1200;
            amounts[i] = payments[i] + percents[i];
        }
